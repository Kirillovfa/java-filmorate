...
public class FilmorateApplicationTests {
    ...
    void testAddAndRemoveFriend() {
        ...
        assertTrue(userService.getFriends(user1.getId()).stream()
                .anyMatch(u -> u.getId() == user2.getId()));
        assertTrue(userService.getFriends(user2.getId()).stream()
                .anyMatch(u -> u.getId() == user1.getId()));

        userService.removeFriend(user1.getId(), user2.getId());
        assertFalse(userService.getFriends(user1.getId()).stream()
                .anyMatch(u -> u.getId() == user2.getId()));
        assertFalse(userService.getFriends(user2.getId()).stream()
                .anyMatch(u -> u.getId() == user1.getId()));
    }
